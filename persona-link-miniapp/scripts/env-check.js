...
assert.match(resolve('develop'), /^http:\/\/[^/]+:8080$/);
assert.equal(resolve('develop', 'http://localhost:9000/'), 'http://localhost:9000');
for (const env of ['trial', 'release']) {
  assert.equal(resolve(env), 'https://persona.sanmao.online');
  assert.equal(resolve(env, 'http://localhost:9000/'), 'https://persona.sanmao.online');
}
const projectConfig = JSON.parse(fs.readFileSync(path.join(__dirname, '../project.config.json'), 'utf8'));
assert.equal(projectConfig.setting.urlCheck, true, '提审配置必须开启合法域名校验');
