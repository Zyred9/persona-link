...
async function run() {
  ...
  assert.equal(loginCount, 1, '首次读取资料自动静默登录');
  ...
  assert.match(profile.data.profileError, /微信登录失败/);
  assert.equal(loginCount, sessions + 1, '登录失败时资料读取报错');
  ...
  assert.equal(profile.data.avatarImage, 'https://example.test/uploads/avatar.png', '已设置头像时不再使用默认图');
  assert.equal(profile.data.profileCustomized, true, '用户上传头像后视为已提供资料');
  assert.ok(storage.get('personaLinkProfileCache'), '用户提供的资料才写本地缓存');
