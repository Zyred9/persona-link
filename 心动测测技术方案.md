...
- t_test 保存题型标识、类型、分类和 `icon_url`；开发环境上传文件写入可配置本地目录，数据库只保存可访问 URL。
...
- 首页分类来自已发布首页配置，只筛选下方全部题型，不影响上方焦点推荐位和推荐题型。
- 配置发布失败时继续读取上一个有效版本。
...
| config_status | 1 草稿，2 已发布，3 已下线 |
...
V1 共建立 24 张表，其中 22 张业务表、2 张身份与会话支撑表。

### 8.3 MySQL 8.x 建表脚本

正式数据库结构以 `persona-link-server/sql/schema.sql` 为权威，演示数据以 `demo-data.sql` 为权威；以下 DDL 是同步快照，出现差异时必须先修改建库脚本，再同步本文档。建库及演示数据脚本已在临时空 MySQL 8.4 数据库执行通过，共建立 24 张表；演示数据包含 1 分类、1 题型、1 版本、1 维度、1 题、2 选项和 3 条结果规则。验证完成后临时数据库已删除。
...
CREATE TABLE IF NOT EXISTS t_test (
    ...
    icon_url VARCHAR(500) NULL COMMENT '题型Icon地址',
    status TINYINT NOT NULL DEFAULT 1 COMMENT '状态：1启用，0禁用',
    deleted TINYINT NOT NULL DEFAULT 0 COMMENT '删除标记：0正常，1删除',
    create_date DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
    update_date DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
...
CREATE TABLE IF NOT EXISTS t_home_config_version (
    ...
    config_status TINYINT NOT NULL DEFAULT 1 COMMENT '配置状态：1草稿，2已发布，3已下线',
    version_note VARCHAR(500) NULL COMMENT '版本说明',
