...
const { markDirty, markSaved, confirmDiscard } = useUnsavedChanges('当前题目尚未保存，确认放弃修改并离开吗？', saving)
...
async function selectQuestion(event: Event) {
  const select = event.target as HTMLSelectElement
  const index = Number(select.value)
  if (index === currentIndex.value) return
  if (!await confirmDiscard('当前题目尚未保存，确认放弃修改并切换题目吗？')) {
    select.value = String(currentIndex.value)
    return
  }
  showQuestion(index)
}
...
async function createNew() {
  if (editorReadOnly.value || saving.value) return
  if (!await confirmDiscard('当前题目尚未保存，确认放弃修改并新增题目吗？')) return
  showQuestion(questions.value.length)
}

async function removeCurrent() {
  if (!currentQuestionId.value || editorReadOnly.value || saving.value) return
  if (!await confirmDiscard('当前题目尚未保存，继续删除将放弃这些修改。确认删除吗？')) return
  saving.value = true
...
<template>
  <section class="question-page">
    <header class="editor-header">
      <div><button class="back-link" type="button" :disabled="saving" @click="router.push('/types')">‹ 返回题型</button><h1>题目编辑 <span>✦</span></h1></div>
      <div class="header-actions">
        <button class="secondary-action" type="button" :disabled="editorLocked" @click="save('保存题目')">{{ saving ? '保存中...' : '保存题目' }}</button>
        <button class="secondary-action" type="button" @click="previewActive = !previewActive">{{ previewActive ? '关闭高亮' : '预览' }}</button>
        <button class="primary-action" type="button" :disabled="editorLocked" @click="createNew">新增题目</button>
      </div>
    </header>
    ...
    <div class="question-layout">
      <article class="question-card">
        ...
        <div class="option-list" @input="markDirty" @change="markDirty">
          <div v-for="(option, index) in options" :key="index" class="option-editor">
            ...
            <input v-model="option.optionText" :aria-label="`选项 ${letters[index]}`" maxlength="60" placeholder="请输入选项内容" :disabled="editorLocked" />
            <select v-if="questionType === 2" v-model.number="option.dimensionId" :aria-label="`选项 ${letters[index]} 计分维度`" :disabled="editorLocked"><option :value="null" disabled>计分维度</option><option v-for="item in version?.dimensions ?? []" :key="item.id" :value="item.id">{{ item.dimensionName }}</option></select>
            <input v-model.number="option.scoreValue" class="option-score" type="number" :aria-label="`选项 ${letters[index]} 分值`" placeholder="分值" :disabled="editorLocked" />
            <button type="button" :disabled="editorLocked" :aria-label="`删除选项 ${letters[index]}`" @click="removeOption(index)">♲</button>
          </div>
        </div>
        <button class="add-option" type="button" :disabled="editorLocked" @click="addOption">＋ 添加选项</button>
        <button v-if="currentQuestionId" class="add-option" type="button" :disabled="editorLocked" @click="removeCurrent">删除当前题目</button>
      </article>
...
.editor-header { display: flex; align-items: flex-end; justify-content: space-between; gap: 22px; }.editor-header h1 { margin: 5px 0 0; font: 700 clamp(34px, 4vw, 46px) var(--font-ui); }.editor-header h1 span { color: #a473df; }.back-link { padding: 0; border: 0; background: transparent; color: #766f78; cursor: pointer; }
