...
<template>
  <section class="feedback-view">
    ...
    <div class="table-card" :aria-busy="loading">
      <table><thead><tr><th>编号</th><th class="submitter">提交用户（昵称 / OpenID）</th><th>反馈内容</th><th>提交时间</th></tr></thead>
        <tbody><tr v-for="item in records" :key="item.id"><td>{{ item.id }}</td><td class="submitter"><span>{{ item.nickname || '匿名' }}</span><small v-if="item.openId">{{ item.openId }}</small></td><td><details><summary>{{ item.content.slice(0, 100) }}{{ item.content.length > 100 ? '…' : '' }}</summary><p class="full-content">{{ item.content }}</p></details></td><td>{{ new Date(item.createdAt).toLocaleString() }}</td></tr></tbody>
...
<style scoped>
.submitter{width:240px;user-select:text;word-break:break-all}.submitter small{display:block;margin-top:6px;color:#8a8391;font-size:12px}.feedback-view table{min-width:720px}
