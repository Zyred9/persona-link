...
<template>
  <section class="config-view">
    ...
    <p class="help">小程序「设置 → 版本信息」读取 <code>miniapp.version</code>（字符串，如 1.0.0），保存后下次进入设置生效。首页标题图也可在「首页配置」上传修改。加入双人测试页头图读取 <code>miniapp.pair.join_hero_image_url</code>（字符串，填写图片地址），留空或图片加载失败时隐藏。小程序「设置 → 联系邮箱」读取 <code>miniapp.contact_email</code>（字符串，须为有效邮箱地址），留空时展示「未配置」。</p>
